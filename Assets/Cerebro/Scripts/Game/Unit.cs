...
namespace Cerebro {
	public abstract class Unit : MonoBehaviour
	{
	    ...
	    public virtual void DealDamage(Unit other)
	    {
	        ...
	        if (ActionPoints == 0)
	        {
				CerebroHelper.DebugLog ("Marking as finished");
	    ...
	    {
			if (isMoving || destinationCell.isInvincible) {
				CerebroHelper.DebugLog ("Cell Invincible");
				return;
			}
			
			var newUnit = Instantiate (gameObject);
			newUnit.transform.SetParent (gameObject.transform.parent);
			newUnit.transform.localScale = gameObject.transform.localScale;

			newUnit.transform.position = destinationCell.transform.position;
...
//	            if (pathCost > 0 && pathCost <= MovementPoints)
		...
		public abstract void MarkAsWaiting();
	    /// <summary>
