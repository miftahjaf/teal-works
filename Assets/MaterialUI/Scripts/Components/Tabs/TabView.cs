...
namespace MaterialUI
{
    ...
    public class TabView : UIBehaviour
    {
        ...
        private AutoTweener m_IndicatorTweener;
        private AutoTweener m_TabsContainerTweener;
        private AutoTweener m_PagesContainerTweener;
        ...
        public void SetPage(int index, bool animate = true)
        {
            ...
            if (m_LowerUnselectedTabAlpha)
            {
                if (animate)
                {
                    ...
                    {
                        TweenManager.AutoTween(m_Tabs[i].gameObject, m_Tabs[i].canvasGroup, "alpha",
                            (i == index ? 1f : 0.5f), 0.5f);
                    }
                }
                else
                {
                    for (int i = 0; i < m_Tabs.Length; i++)
                    {
                        m_Tabs[i].canvasGroup.alpha = (i == index ? 1f : 0.5f);
                    }
                }
            }
        }
        ...
        private void TweenPagesContainer(int index, bool animate = true)
        {
            ...
            if (m_PagesContainerTweener != null)
            {
                Destroy(m_PagesContainerTweener);
            }
            ...
            if (animate)
            {
                m_PagesContainerTweener = TweenManager.AutoTween(m_PagesContainer.gameObject, m_PagesContainer,
                    "anchoredPosition", new Vector2(targetPosition, 0), 0.5f, callback: OnPagesTweenEnd);
            }
            else
            {
        ...
        private void TweenTabsContainer(int index, bool animate = true)
        {
            ...
            if (m_TabsContainerTweener != null)
            {
                Destroy(m_TabsContainerTweener);
            }

            if (animate)
            {
                m_TabsContainerTweener = TweenManager.AutoTween(m_TabsContainer.gameObject, m_TabsContainer,
                    "anchoredPosition", new Vector2(targetPosition, 0), 0.5f);
            }
            else
            {
        ...
        private void TweenIndicator(int targetTab, bool animate = true)
        {
            ...
            if (m_IndicatorTweener != null)
            {
                Destroy(m_IndicatorTweener);
            }

            if (animate)
            {
                m_IndicatorTweener = TweenManager.AutoTween(m_Indicator.gameObject, m_Indicator,
                    "anchoredPosition", new Vector2(targetPosition, 0), 0.5f);
            }
            else
            {
