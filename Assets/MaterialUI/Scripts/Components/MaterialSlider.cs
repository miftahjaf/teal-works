...
namespace MaterialUI
{
    ...
    public class MaterialSlider : UIBehaviour, ISelectHandler, IDeselectHandler, IPointerDownHandler, IPointerUpHandler, ILayoutGroup, ILayoutElement
    {
        ...
        private AutoTweener m_HandleSizeTweener;
        private AutoTweener m_PopupScaleTweener;
        //private AutoTweener m_PopupPositionTweener;

        private AutoTweener m_HandleAnchorMinTweener;
        private AutoTweener m_HandleAnchorMaxTweener;
        private AutoTweener m_HandlePositionYTweener;

        private AutoTweener m_PopupTextColorTweener;
        ...
        public void OnEditorUpdate()
        {
            ...
            //m_HandleTracker.Add(this, handleGraphicTransform, DrivenTransformProperties.AnchoredPosition3D);
            //handleGraphicTransform.position = m_Slider.handleRect.position;

            //m_HandleTracker.Add(this, handleGraphicTransform, DrivenTransformProperties.AnchorMin);
            //m_HandleTracker.Add(this, handleGraphicTransform, DrivenTransformProperties.AnchorMax);
            ...
            if (slider.wholeNumbers && m_HasDots)
            {
                ...
                for (int i = 0; i < m_DotGraphics.Length; i++)
                {
                    if (m_DotGraphics[i] != null)
                    {
                        if (slider.value > i)
                        {
                            m_DotGraphics[i].color = slider.interactable ? m_EnabledColor : m_DisabledColor;
                        }
                        else
                        {
            ...
            if (m_HandleGraphic)
            {
                m_HandleGraphic.color = slider.interactable ? m_EnabledColor : m_DisabledColor;
            }

            canvasGroup.interactable = slider.interactable;
            canvasGroup.blocksRaycasts = slider.interactable;

            if (m_InputField)
            {
                m_InputField.interactable = slider.interactable;
            }
        }
#endif
        ...
        {
            if (m_HandleSizeTweener != null)
            {
                Destroy(m_HandleSizeTweener);
            }
            /*if (m_PopupPositionTweener != null)
            {
                Destroy(m_PopupPositionTweener);
            }*/
            if (m_PopupScaleTweener != null)
            {
                Destroy(m_PopupScaleTweener);
            }
            if (m_HandlePositionYTweener != null)
            {
                Destroy(m_HandlePositionYTweener);
            }
            if (m_PopupTextColorTweener != null)
            {
                Destroy(m_PopupTextColorTweener);
            }

            m_HandleSizeTweener = TweenManager.AutoTween(handleGraphicTransform.gameObject, handleGraphicTransform, "sizeDelta", new Vector2(38, 38), m_AnimationDuration, 0, Tween.TweenType.SoftEaseOutQuint);
            m_PopupScaleTweener = TweenManager.AutoTween(m_PopupTransform.gameObject, m_PopupTransform, "localScale", Vector3.one, m_AnimationDuration, 0, Tween.TweenType.EaseOutSept);
            m_HandlePositionYTweener = TweenManager.AutoTween(m_HandleGraphicTransform.gameObject,
                m_HandleGraphicTransform, "anchoredPosition", new Vector2(0, (slider.wholeNumbers && m_HasDots) ? 36 : 30), m_AnimationDuration, 0, Tween.TweenType.EaseOutSept, subvaluesToModify: new[] { false, true });

            m_PopupTextColorTweener = TweenManager.AutoTween(m_PopupText.gameObject, m_PopupText, "color",
        ...
        private void AnimateOff()
        {
            ...
            m_HandleSizeTweener = TweenManager.AutoTween(handleGraphicTransform.gameObject, handleGraphicTransform, "sizeDelta", new Vector2(16, 16), m_AnimationDuration, 0, Tween.TweenType.EaseOutSept);
            m_PopupScaleTweener = TweenManager.AutoTween(m_PopupTransform.gameObject, m_PopupTransform, "localScale", Vector3.zero, m_AnimationDuration);
//            m_HandlePositionYTweener = TweenManager.AutoTween(m_HandleGraphicTransform.gameObject, m_HandleGraphicTransform, "anchoredPosition", new Vector2(0, (MaterialUIScaler.GetParentScaler(transform).targetCanvas.pixelPerfect) ? 1f : 0f), m_AnimationDuration, 0, Tween.TweenType.EaseOutCubed, subvaluesToModify: new[] { false, true });
        ...
        {
            if (m_HandleAnchorMinTweener != null)
            {
                Destroy(m_HandleAnchorMinTweener);
            }

            if (m_HandleAnchorMaxTweener != null)
            {
                Destroy(m_HandleAnchorMaxTweener);
            }

            if (slider.wholeNumbers && SliderValueRange() < 100)
            {
                m_HandleAnchorMinTweener = TweenManager.AutoTween(handleGraphicTransform.gameObject,
                    handleGraphicTransform, "anchorMin", m_Slider.handleRect.anchorMin, m_AnimationDuration * 0.5f,
                    subvaluesToModify: new[] { true, false }, tweenType: Tween.TweenType.EaseOutSept);

                m_HandleAnchorMaxTweener = TweenManager.AutoTween(handleGraphicTransform.gameObject,
                    handleGraphicTransform, "anchorMax", m_Slider.handleRect.anchorMax, m_AnimationDuration * 0.5f,
                    subvaluesToModify: new[] { true, false }, tweenType: Tween.TweenType.EaseOutSept);
            }
            else
            {
        ...
        public void CalculateLayoutInputHorizontal()
        {
            ...
            float dotsSize = (slider.wholeNumbers && m_HasDots) ? 3f : 0f;

            m_LeftWidth += dotsSize;
            m_RightWidth += (dotsSize * 2);

            m_Width = Mathf.Max(m_ManualPreferredWidth, m_LeftWidth + m_RightWidth);
        }
        ...
        public void SetLayoutHorizontal()
        {
            ...
            m_SliderContentTransform.anchoredPosition = new Vector2(m_LeftWidth, m_SliderContentTransform.anchoredPosition.y);
            m_SliderContentTransform.sizeDelta = new Vector2(-(m_LeftWidth + m_RightWidth), m_SliderContentTransform.sizeDelta.y);
        }
