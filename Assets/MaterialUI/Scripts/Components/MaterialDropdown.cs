...
namespace MaterialUI
{
    ...
    public class MaterialDropdown : UIBehaviour, IOptionDataListContainer
    {
        ...
        private List<AutoTweener> m_AutoTweeners;
        ...
        public void Show()
        {
            GameObject dropdownCanvasGameObject = GameObject.Find("Dropdown Canvas");

            if (dropdownCanvasGameObject == null)
            {
                dropdownCanvasGameObject = new GameObject("Dropdown Canvas");
            }

            Canvas dropdownCanvas = dropdownCanvasGameObject.GetAddComponent<Canvas>();

            Canvas mainCanvas = m_BaseTransform.root.GetComponent<Canvas>();

            RectTransform mainCanvasRectTransform = mainCanvas.GetComponent<RectTransform>();
            RectTransform dropdownCanvasRectTransform = dropdownCanvas.GetComponent<RectTransform>();
            ...
            dropdownCanvas.gameObject.GetAddComponent<GraphicRaycaster>();
            CanvasScaler mainScaler = mainCanvas.GetComponent<CanvasScaler>();
            if (mainScaler != null)
            {
                CanvasScaler scaler = dropdownCanvas.gameObject.GetAddComponent<CanvasScaler>();
            ...
            dropdownCanvas.gameObject.GetAddComponent<MaterialUIScaler>();
            dropdownCanvas.pixelPerfect = true;
            dropdownCanvas.sortingOrder = 30000;
            dropdownCanvas.renderMode = mainCanvas.renderMode;

            m_DropdownPanel = PrefabManager.InstantiateGameObject(PrefabManager.ResourcePrefabs.dropdownPanel, dropdownCanvas.transform).GetComponent<RectTransform>();
            ...
            if (m_ExpandStartType == ExpandStartType.ExpandFromBaseTransformSize)
            {
            ...
            m_AutoTweeners = new List<AutoTweener>();

            m_AutoTweeners.Add(TweenManager.AutoTween(m_DropdownCanvasGroup.gameObject, m_DropdownCanvasGroup, "alpha", 1f, m_AnimationDuration * 0.66f, 0f, Tween.TweenType.Linear));
            m_AutoTweeners.Add(TweenManager.AutoTween(m_ShadowCanvasGroup.gameObject, m_ShadowCanvasGroup, "alpha", 1f, m_AnimationDuration * 0.66f, 0f, Tween.TweenType.Linear));

            m_AutoTweeners.Add(TweenManager.AutoTween(m_DropdownPanel.gameObject, m_DropdownPanel, "sizeDelta", m_ExpandedSize, m_AnimationDuration, m_AnimationDuration / 3, Tween.TweenType.EaseInOutQuint));

            m_AutoTweeners.Add(TweenManager.AutoTween(m_DropdownPanel.gameObject, m_DropdownPanel, "position", m_ExpandedPosition, m_AnimationDuration, m_AnimationDuration / 3, Tween.TweenType.EaseInOutQuint, callback: () =>
            {
            ...
            for (int i = 0; i < m_ListItems.Count; i++)
            {
                TweenManager.AutoTween(m_ListItems[i].canvasGroup.gameObject, m_ListItems[i].canvasGroup, "alpha", 1f, m_AnimationDuration * 1.66f, (i * (m_AnimationDuration / 6)) + m_AnimationDuration, Tween.TweenType.Linear);
            }
        ...
        public void Hide()
        {
            ...
            for (int i = 0; i < m_ListItems.Count; i++)
            {
                m_AutoTweeners.Add(TweenManager.AutoTween(m_ListItems[i].canvasGroup.gameObject, m_ListItems[i].canvasGroup, "alpha", 0f, m_AnimationDuration * 0.66f, ((m_ListItems.Count - i) * (m_AnimationDuration / 6)), Tween.TweenType.Linear));
            }

            m_AutoTweeners.Add(TweenManager.AutoTween(m_DropdownCanvasGroup.gameObject, m_DropdownCanvasGroup, "alpha", 0f, m_AnimationDuration * 0.66f, m_AnimationDuration, Tween.TweenType.Linear));

            TweenManager.AutoTween(m_ShadowCanvasGroup.gameObject, m_ShadowCanvasGroup, "alpha", 0f, m_AnimationDuration * 0.66f, m_AnimationDuration, Tween.TweenType.Linear, callback: () =>
            {
                for (int i = 0; i < m_AutoTweeners.Count; i++)
                {
                    if (m_AutoTweeners[i] != null)
                    {
                        Destroy(m_AutoTweeners[i]);
                    }
                }
        ...
        private Vector3 CalculatedPosition()
        {
            ...
            int flipper = m_VerticalPivotType == VerticalPivotType.FirstItem ? 1 : -1;
        ...
        private DropdownListItem CreateItem(OptionData data, int index)
        {
            ...
            else if (item.image is VectorImage)
            {
                if (data.imageData != null && data.imageData.vectorImageData != null)
                {
                    item.image.SetImage(data.imageData);
                }
                else
            ...
            itemGameObject.GetChildByName<Text>("Text").color = m_ItemTextColor;
            itemGameObject.GetChildByName<Graphic>("Icon").color = m_ItemIconColor;
