...
namespace MaterialUI
{
    ...
    public class MaterialInputField : UIBehaviour, ILayoutGroup, ILayoutElement, ISelectHandler, IDeselectHandler
    {
        ...
        private AutoTweener m_LeftContentTweener;
        private AutoTweener m_RightContentTweener;
        private AutoTweener m_HintTextTweener;
        private AutoTweener m_ValidationColorTweener;
        private AutoTweener m_CounterTweener;
        ...
        private AutoTweener m_ActiveLinePosTweener;
        private AutoTweener m_ActiveLineSizeTweener;
        private AutoTweener m_ActiveLineAlphaTweener;
        private AutoTweener m_HintTextFloatingValueTweener;
        private AutoTweener m_ValidationTweener;
        ...
        private void ValidateText()
        {
            ...
            {
                if (m_ValidationTweener != null)
                {
                    Destroy(m_ValidationTweener);
                }

                if (validator.ValidateText(inputField.text, validationText))
                {
                    if (Application.isPlaying)
                    {
                        ...
                        m_ValidationTweener = TweenManager.AutoTween(validationCanvasGroup.gameObject, validationCanvasGroup, "alpha", 1f, m_AnimationDuration / 2, tweenType: Tween.TweenType.Linear);
                        return;
                    }
                }
                else
                {
                    if (Application.isPlaying)
                    {
                        m_ValidationTweener = TweenManager.AutoTween(validationCanvasGroup.gameObject, validationCanvasGroup, "alpha", 0f, m_AnimationDuration / 2, tweenType: Tween.TweenType.Linear, callback: () =>
                        {
        ...
        {
            if (m_LastInteractableState != inputField.interactable)
            {
                m_LastInteractableState = inputField.interactable;
                UpdateSelectionState();
            }
        }
        ...
        private void UpdateSelectionState()
        {
            ...
            if (m_CurrentSelectionState != m_LastSelectionState)
            {
                ...
                if (m_LeftContentTweener)
                {
                    Destroy(m_LeftContentTweener);
                }
                if (Application.isPlaying)
                {
                    if (m_LeftContentGraphic)
                    {
                        m_LeftContentTweener = TweenManager.AutoTween(m_LeftContentGraphic.gameObject,
                ...
                if (m_RightContentTweener)
                {
                    Destroy(m_RightContentTweener);
                }
                if (Application.isPlaying)
                {
                    if (m_RightContentGraphic)
                    {
                        m_RightContentTweener = TweenManager.AutoTween(m_RightContentGraphic.gameObject,
                ...
                if (m_HintTextTweener)
                {
                    Destroy(m_HintTextTweener);
                }
                if (Application.isPlaying)
                {
                    m_HintTextTweener = TweenManager.AutoTween(m_HintTextObject.gameObject, m_HintTextObject,
                        "color",
                        IsSelected() ? m_HintTextActiveColor : m_HintTextInactiveColor, m_AnimationDuration);
                }
                else
                {
                    m_HintTextObject.color = IsSelected() ? m_HintTextActiveColor : m_HintTextInactiveColor;
                ...
                if (m_CounterTweener)
                {
                    Destroy(m_CounterTweener);
                }
                if (Application.isPlaying)
                {
                    m_CounterTweener = TweenManager.AutoTween(m_CounterText.gameObject, m_CounterText,
                        "color",
                        IsSelected() ? m_CounterActiveColor : m_CounterInactiveColor, m_AnimationDuration);
                }
                else
                {
                    m_CounterText.color = IsSelected() ? m_CounterActiveColor : m_CounterInactiveColor;
                ...
                if (m_ValidationColorTweener)
                {
                    Destroy(m_ValidationColorTweener);
                }
                if (Application.isPlaying)
                {
                    m_ValidationColorTweener = TweenManager.AutoTween(m_ValidationText.gameObject, m_ValidationText,
                        "color",
                        IsSelected() ? m_ValidationActiveColor : m_ValidationInactiveColor, m_AnimationDuration);
                }
                else
                {
                    m_ValidationText.color = IsSelected() ? m_ValidationActiveColor : m_ValidationInactiveColor;
        ...
        private void AnimateHintTextSelect()
        {
            ...
            m_HintTextFloatingValueTweener = TweenManager.AutoTween(gameObject, this, "hintTextFloatingValue", 1f, m_AnimationDuration, callback: () =>
                {
        ...
        private void AnimateHintTextDeselect()
        {
            ...
            {
                m_HintTextFloatingValueTweener = TweenManager.AutoTween(gameObject, this, "hintTextFloatingValue", 0f, m_AnimationDuration, callback: () =>
                {
                    m_AnimateHintText = false;
                    SetHintLayoutToFloatingValue();
                });
        ...
        private void AnimateActiveLineSelect(bool instant = false)
        {
            ...
            else
            {
                ...
                m_ActiveLinePosTweener = TweenManager.AutoTween(m_ActiveLineTransform.gameObject,
                    m_ActiveLineTransform, "anchoredPosition", Vector2.zero, m_AnimationDuration * 2,
                    subvaluesToModify: new[] { true, false });

                m_ActiveLineSizeTweener = TweenManager.AutoTween(m_ActiveLineTransform.gameObject,
                    m_ActiveLineTransform, "sizeDelta", new Vector2(m_LineTransform.GetProperSize().x, 0), m_AnimationDuration * 2,
                    subvaluesToModify: new[] { true, false });
            }
        }
        ...
        private void AnimateActiveLineDeselect(bool instant = false)
        {
            ...
            else
            {
                ...
                m_ActiveLineAlphaTweener = TweenManager.AutoTween(activeLineCanvasGroup.gameObject,
                    activeLineCanvasGroup, "alpha", 0f, m_AnimationDuration * 2);
            }
        }

        private void CancelHintTextTweeners()
        {
            CancelTweener(m_HintTextFloatingValueTweener);
            m_AnimateHintText = false;
        }

        private void CancelActivelineTweeners()
        {
            CancelTweener(m_ActiveLineSizeTweener);
            CancelTweener(m_ActiveLinePosTweener);
            CancelTweener(m_ActiveLineAlphaTweener);
        }

        private void CancelTweener(AutoTweener tweener)
        {
            if (tweener != null)
            {
                Destroy(tweener);
        ...
        {
            if (m_HintTextFloatingValueTweener == null)
            {
