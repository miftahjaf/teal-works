...
namespace MaterialUI
{
    public class VectorImagePickerWindow : EditorWindow
    {
        ...
        private static VectorImageData m_VectorImageData;
        private static Object m_ObjectToRefresh;
        ...
        private void DrawPicker()
        {
            ...
            if (m_VectorImageData.glyph == null)
            {
                GUILayout.Label("Invalid glyph");
                return;
            }

            if (m_VectorImageData.font == null)
            {
                m_VectorImageData.font = VectorImageManager.GetIconFont(VectorImageManager.GetAllIconSetNames()[1]);
            }

            string[] names = VectorImageManager.GetAllIconSetNames();

            if (!names.Contains(m_VectorImageData.font.name))
            {
                m_VectorImageData.font.name = names[1];
            }
            ...
            if (VectorImageManager.GetAllIconSetNames().Length > 0)
            {
                ...
                m_VectorImageData.font = VectorImageManager.GetIconFont(names[EditorGUILayout.Popup(new GUIContent("Current Pack"), names.ToList().IndexOf(m_VectorImageData.font.name), namesContents)]);
        ...
        private void DrawIconList()
        {
            ...
            GUILayout.Space(5f);
            m_IconViewScrollPosition = GUILayout.BeginScrollView(m_IconViewScrollPosition, GUILayout.Height(Screen.height - 80f));
		...
		private void SetGlyph(int index)
		{
			if (m_VectorImageData != null)
            {
                if (m_ObjectToRefresh != null)
                {
                    Undo.RecordObject(m_ObjectToRefresh, "pick icon");
                }

                m_VectorImageData.glyph = new Glyph(m_GlyphArray[index].name, m_GlyphArray[index].unicode, true);
				m_VectorImageData.font = m_IconFont;
			}
		}
    }
}