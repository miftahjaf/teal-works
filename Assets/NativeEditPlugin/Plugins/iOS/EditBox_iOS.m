...
-(void)onKeyboardChanged : (BOOL*)show
{
    JsonObject* jsonToUnity = [[JsonObject alloc] init];
    
    [jsonToUnity setString:@"msg" value:MSG_KEYBOARD];
    [jsonToUnity setBool:@"show" value:show];
    [jsonToUnity setFloat:@"keyheight" value:[self getKeyboardheight]];
...
-(void) onTextEditEnd:(NSString*) text
{
    ...
    jsonToUnity = [[JsonObject alloc] init];
...
-(void) textFieldDidChange :(UITextField *)textField{
    ...
    if(characterLimit>0)
    {
        ...
        if(textLength > characterLimit) {
            textField.text = [textField.text substringToIndex:characterLimit];
        }
    }
