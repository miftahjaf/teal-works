...
namespace TexDrawLib
{
    public class MatrixAtom : Atom
    {
        ...
        public override Box CreateBox(TexStyle style)
        {
            ...
            bool outsideGap = horizontalLine > 0 || verticalLine > 0;
            Box resultBox = ObjPool<VerticalBox>.Get();
	        Box kern = null;
	        Box kernHalf = null;
            ...
            if (verticalLine > 0) {
                ...
                for (int i = 0; i < boxesHeight.Count; i++) {
                    if (i == 0) {
                        ...
                        resultBox.Add(StrutBox.Get(0, boxesHeight[i] + gapOutside, 0, 0));
                        continue;
                    }
                    if (i == 1) {
                        if (first)
                            resultBox.Add(HorizontalRule.Get(firstThick ? lineThick * 2 : lineThick, size, 0));
                        resultBox.Add(StrutBox.Get(0, boxesHeight[i] + (first ? (firstThick ? gapXThick : gapX) : gapXNone), 0, 0));
                        continue;
                    }
                    if (inset)
                        resultBox.Add(HorizontalRule.Get(lineThick, size, 0));
                    resultBox.Add(StrutBox.Get(0, boxesHeight[i] + gapInset, 0, 0));
                }
                if (outside)
